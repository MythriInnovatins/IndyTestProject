...
//module.exports.SetOwnershipStewardsVerinym = SetOwnershipStewardsVerinym;
//module.exports.ConnectToIndyPool = ConnectToIndyPool;
//module.exports.ClientOnBoarding = ClientOnBoarding;
module.exports.CreateScemaAndDef = CreateScemaAndDef;
module.exports.EmployeeOnboarding = EmployeeOnboarding;
module.exports.CredentialOfferForEmployee = CredentialOfferForEmployee;
module.exports.CredentialRequestToClient = CredentialRequestToClient;
...
exports.DeleteWallet = async function(WalletConfig, Credentials)
{
    let clientWalletConfig = JSON.parse(WalletConfig);
    let clientWalletCred = JSON.parse(Credentials);
    let wallet;
    console.log(clientWalletConfig)
    console.log(clientWalletCred)
    await GetWallet(WalletConfig, Credentials).then(data=>{
        wallet = data;
    });

    await indy.closeWallet(wallet);
    await indy.deleteWallet(clientWalletConfig, clientWalletCred).then(data=>{
        return "wallet deleted";
    })

}

exports.ConnectToIndyPool = async function (poolName) {
    
    let poolHandle;
    let poolGenesisTxnPath;
    await util.getPoolGenesisTxnPath(poolName).then(data=>{
        poolGenesisTxnPath = data;
    });
...
//This method will be invoked by IDRAMP.
exports.SetOwnershipStewardsVerinym = async function(stewardWalletConfig, stewardWalletCredentials,stewardDidInfo) {
    ...
    //let stewardWalletConfig1 = {'id': 'stewardWalletName'}
    //let stewardWalletCredentials1 = {'key': 'steward_key'}
    let WalletConfig = JSON.parse(stewardWalletConfig);
    let WalletCredentials = JSON.parse(stewardWalletCredentials);
    let stewardWallet;
    ...
    await GetWallet(stewardWalletConfig, stewardWalletCredentials).then(data=>{
        stewardWallet = data;
        return `Wallet already exists.wallet id is ${stewardWallet}`;
    });
    ...
    console.log(stewardWallet);
    console.log("\"Sovrin Steward\" -> Create and store in Wallet DID from seed");
    // let stewardDidInfo = {
    //     'seed': '000000000000000000000000Steward1'
    // };
    let stewardInfo = JSON.parse(stewardDidInfo);
    let stewardDid, stewardKey;
    await indy.createAndStoreMyDid(stewardWallet, stewardInfo).then(data=>{
        console.log(data)
        stewardDid = data[0];
        stewardKey = data[1];
    ...
    // indy.closeWallet(stewardWallet);
    // indy.deleteWallet(stewardWalletConfig, stewardWalletCredentials);

    //console.log("Close and Delete pool");

    var didValue = { 'stewardDid': stewardDid, 'stewardKey': stewardKey };

    return didValue;
}

//This method will be invoked by steward.
exports.ClientOnBoarding = async function(clientName, poolName, stewardDid, 
    clientWalletConfig, clientWalletCredentials,stewardWalletConfig,
    stewardWalletCredentials) {
    let retValue;
    let poolHandle;
    let walletConfig = JSON.parse(clientWalletConfig);
    let walletCredentials = JSON.parse(clientWalletCredentials);
    ...
    await GetPoolHandle(poolName).then(data=>{
        poolHandle = data;
    }).catch(e=>{
        throw e;
    });
   // console.log(poolHandle);
   console.log(`stewardWallet ${stewardWallet}`)

    let clientWallet, stewardClientKey, clientStewardDid, clientStewardKey;
    await onboarding(poolHandle, "Sovrin Steward", stewardWallet, stewardDid,
    clientName, null, walletConfig, walletCredentials).then(data=>{
        clientWallet = data[0];
        stewardClientKey = data[1];
        clientStewardDid = data[2];
        clientStewardKey = data[3];
    }).catch(e=>{
        throw e;
    });

    // var didValue = { 'clientWallet': clientWallet, 'stewardClientKey': stewardClientKey,
    // 'clientStewardDid': clientStewardDid,'clientStewardKey': clientStewardKey};
    // console.log("== Getting Trust Anchor credentials");
    let clientDid;
    await getVerinym(poolHandle, "Sovrin Steward", stewardWallet, 
    stewardDid,stewardClientKey, clientName, clientWallet, clientStewardDid,
    clientStewardKey, 'TRUST_ANCHOR').then(data=>{
        clientDid = data;
    }).catch(e=>{
        throw e;
    });

    var didValue = {'clientDid':clientDid};
    return didValue;
}

//This method will be invoked by the client.
async function CreateScemaAndDef(poolName,clientDid,clientWalletConfig,clientWalletCredentials,schemaName){
    ...
    await GetWallet(clientWalletConfig,clientWalletCredentials).then(data=>{
        clientWallet = data;
    }).catch(e=>{
        if(e.message != 'WalletAlreadyOpenedError')
        {
    ...
    let transcriptSchemaId;
    let transcriptSchema; 

    await indy.issuerCreateSchema(clientDid, schemaName, '1.2',
    ['first_name', 'last_name', 'degree', 'status',
    'year', 'average', 'ssn']
).then(data=>{
                
                transcriptSchemaId = data[0];
                transcriptSchema = data[1];
            }).catch(e=>{
                throw e;
            });
            //console.log(transcriptSchemaId);
            console.log(transcriptSchema);
            //console.log(poolHandle);
            //console.log(clientWallet);
    console.log("\"Client\" -> Send \"Transcript\" Schema to Ledger");
    await sendSchema(poolHandle, clientWallet, clientDid,transcriptSchema);

    console.log("==============================");
    console.log("=== Client Credential Definition Setup ==");
    console.log("------------------------------");

    console.log("\"Client\" -> Get \"Transcript\" Schema from Ledger");
    [, transcriptSchema] = await getSchema(poolHandle, clientDid, transcriptSchemaId);

    console.log("\"Client\" -> Create and store in Wallet \"Client Transcript\" Credential Definition");
    let [clientTranscriptCredDefId, clientTranscriptCredDefJson] = await indy.issuerCreateAndStoreCredentialDef(clientWallet, clientDid, transcriptSchema, 'TAG1', 'CL', '{"support_revocation": false}');

    console.log("\"Client\" -> Send  \"Client Transcript\" Credential Definition to Ledger");
    await sendCredDef(poolHandle, clientWallet, clientDid, clientTranscriptCredDefJson);
    
    var didValue = { 'transcriptSchemaId':transcriptSchemaId,'clientTranscriptCredDefId':clientTranscriptCredDefId,'clientTranscriptCredDefJson':clientTranscriptCredDefJson};
    return didValue;
}

//This method will be invoked by the client.
async function EmployeeOnboarding(poolName,clientName,emplName,clientDid,emplWalletConfig,emplWalletCredentials,
    clientWalletConfig,clientWalletCred)
{
    console.log(emplWalletConfig);
    console.log(emplWalletCredentials);
    let walletConfig = JSON.parse(emplWalletConfig);
    let walletCredentials = JSON.parse(emplWalletCredentials);
    let clientWalletConfigVal = JSON.parse(clientWalletConfig);
    let clientWalletCredVal = JSON.parse(clientWalletCred);
    let poolHandle;
    let clientWallet;
    indy.setProtocolVersion(2);
    ...
    {
        console.log(poolName);
        await indy.openPoolLedger(poolName).then(data=>{
            poolHandle = data;
        });
        console.log(poolHandle);
    }
    ...
    try
    {
        await indy.openWallet(clientWalletConfigVal, clientWalletCredVal).then(data=>{
            clientWallet = data;
        })
    }
    catch(e){
    ...
    let [emplWallet, clientEmplKey, emplClientDid, emplClientKey, 
        clientEmplConnectionResponse] = 
    await onboarding(poolHandle, clientName, clientWallet, clientDid, emplName, 
    null, walletConfig , walletCredentials);

    let didValue = { 'emplWallet':emplWallet,'clientEmplKey':clientEmplKey,
        'emplClientDid':emplClientDid,'emplClientKey':emplClientKey,'clientEmplConnectionResponse':clientEmplConnectionResponse}
    return didValue;
}

//This method will be invoked by the client.
async function CredentialOfferForEmployee(poolName,clientWalletConfig,clientWalletCred,
    clientTranscriptCredDefId,clientEmplConnectionResponseDid,
    clientEmplKey,emplClientVerkey)
{
    //console.log("test");
    ...
    //     throw e;
    // }
    await ConnectToIndyPool(poolName).then(data=>{
        poolHandle = data;
    }).catch(e=>{
        throw e;
    });
    
    await GetWallet(clientWalletConfig,clientWalletCred).then(data=>{
        clientWallet = data;
    }).catch(e=>{
        if(e.message != 'WalletAlreadyOpenedError')
        {
    ...
    let transcriptCredOfferJson;
    // console.log("\"Client\" -> Create \"Transcript\" Credential Offer for Alice");
    await indy.issuerCreateCredentialOffer(clientWallet, clientTranscriptCredDefId).then(data=>{
         transcriptCredOfferJson = data;
    });
    
    // console.log("\"Client\" -> Get key for Alice did");
    let emplClientVerkey1;
    await indy.keyForDid(poolHandle, clientWallet, clientEmplConnectionResponseDid).then(data=>{
        emplClientVerkey1 = data;
    });

    // console.log("\"Client\" -> Authcrypt \"Transcript\" Credential Offer for Alice");
    let authcryptedTranscriptCredOffer1 ;
     await indy.cryptoAuthCrypt(clientWallet, clientEmplKey, emplClientVerkey, 
        Buffer.from(JSON.stringify(transcriptCredOfferJson), 'utf8')).then(data=>{
            authcryptedTranscriptCredOffer1 = data;
        });
      
        //Send this authcryptedTranscriptCredOffer to employee
        let did = {'transcriptCredOfferJson':transcriptCredOfferJson,'emplClientVerkey':emplClientVerkey1,
                    'authcryptedTranscriptCredOffer':authcryptedTranscriptCredOffer1};
        return did;
...
async function GetWallet(walletConfig,walletCred)
{
    let walletConfigVal = JSON.parse(walletConfig);
    let walletCredVal = JSON.parse(walletCred);
    let usrWallet;
    console.log(walletConfigVal);
    console.log(walletCredVal);
    try
    {
        await indy.openWallet(walletConfigVal, walletCredVal).then(data=>{
            usrWallet = data;
        }).catch(e=>{
            throw e;
        });
...
//This method will be invoked by the employee.
async function CredentialRequestToClient(poolName,emplWalletConfig,emplWalletCred,emplClientKey,
    authcryptedTranscriptCredOffer,emplClientDid)
{
    let emplWallet;
    let poolHandle;
    indy.setProtocolVersion(2);
    await GetWallet(emplWalletConfig,emplWalletCred).then(data=>{
        emplWallet = data;
    }).catch(e=>{
        throw e;
    });
    ...
    console.log("\"Employee\" -> Authdecrypted \"Transcript\" Credential Offer from client");
    let clientEmplVerkey;
    let authdecryptedTranscriptCredOfferJson;
    let authdecryptedTranscriptCredOffer;
    let bufferOriginal = 
    Buffer.from(JSON.parse(authcryptedTranscriptCredOffer));
    console.log(bufferOriginal);
    await authDecrypt(emplWallet,emplClientKey, bufferOriginal).then(data=>{
        clientEmplVerkey = data[0];
        authdecryptedTranscriptCredOfferJson = data[1];
        authdecryptedTranscriptCredOffer = data[2];
        
    }).catch(e=>{
        throw e;
    });

    console.log(authdecryptedTranscriptCredOffer);

    console.log("\"Employee\" -> Create and store \"Employee\" Master Secret in Wallet");
    let emplMasterSecretId = await indy.proverCreateMasterSecret(emplWallet, null);

    console.log("\"Employee\" -> Get \"client Transcript\" Credential Definition from Ledger");

    let clientTranscriptCredDefId;
    let clientTranscriptCredDef;
   
    await getCredDef(poolHandle, emplClientDid, authdecryptedTranscriptCredOffer['cred_def_id']).then(data=>{
        clientTranscriptCredDefId = data[0];
        clientTranscriptCredDef = data[1];
     });

    console.log("\"Employee\" -> Create \"Transcript\" Credential Request for client");
    let [transcriptCredRequestJson, transcriptCredRequestMetadataJson] = 
    await indy.proverCreateCredentialReq(emplWallet, emplClientDid, authdecryptedTranscriptCredOfferJson, 
        clientTranscriptCredDef, emplMasterSecretId);

    console.log("\"Employee\" -> Authcrypt \"Transcript\" Credential Request for client");
    let authcryptedTranscriptCredRequest = await indy.cryptoAuthCrypt(emplWallet, emplClientKey,
         clientEmplVerkey, Buffer.from(JSON.stringify(transcriptCredRequestJson), 'utf8'));

         console.log(authcryptedTranscriptCredRequest);

    console.log("\"Employee\" -> Send authcrypted \"Transcript\" Credential Request to client");
    let did ={'authcryptedTranscriptCredRequest':authcryptedTranscriptCredRequest,'transcriptCredRequestMetadataJson':transcriptCredRequestMetadataJson,'clientTranscriptCredDef':clientTranscriptCredDef}
    return did;
}

//This method will be invoked by the client.
//TODO:add [transcriptCredValues] as parameter in this method.
async function CreateCredentials(clientWalletConfig,clientWalletCred,
    authcryptedTranscriptCredRequest,clientEmplKey,transcriptCredOfferJson)
{
    let clientWallet;
    console.log(clientWalletConfig);
    console.log(clientWalletCred);
    ...
    console.log("\"client\" -> Authdecrypt \"Transcript\" Credential Request from employee");

    let authdecryptedTranscriptCredRequestJson;
    let emplClientVerkey;

    let bufferOriginal = Buffer.from(JSON.parse(authcryptedTranscriptCredRequest));
    
    await authDecrypt(clientWallet, clientEmplKey, bufferOriginal).then(data=>{
         emplClientVerkey = data[0];
         authdecryptedTranscriptCredRequestJson = data[1];
    });

    console.log("\"client\" -> Create \"Transcript\" Credential for employee");
    // note that encoding is not standardized by Indy except that 32-bit integers are encoded as themselves. IS-786
    let transcriptCredValues = {
        "first_name": { "raw": "Smith", "encoded": "1139481716457488690172217916278103335" },
        "last_name": { "raw": "james", "encoded": "5321642780241790123587902456789123452" },
    ...
    let transcriptCredJson;
    console.log('clientWallet');
    console.log(clientWallet);
    console.log('transcriptCredOfferJson');
    console.log(transcriptCredOfferJson);
    console.log('authdecryptedTranscriptCredRequestJson');
    console.log(authdecryptedTranscriptCredRequestJson);
    console.log('transcriptCredValues');
    console.log(transcriptCredValues);

    await indy.issuerCreateCredential(clientWallet, transcriptCredOfferJson,
         authdecryptedTranscriptCredRequestJson, transcriptCredValues, null, -1).then(data=>{
            transcriptCredJson=data;
         });

    console.log("\"client\" -> Authcrypt \"Transcript\" Credential for employee");
    let authcryptedTranscriptCredJson;
    
    await indy.cryptoAuthCrypt(clientWallet,
         clientEmplKey, emplClientVerkey, Buffer.from(JSON.stringify(transcriptCredJson), 'utf8')).then(data=>{
            authcryptedTranscriptCredJson = data;
         });

   ////Create credential request and send to the employee.
   console.log("\"client\" -> Send authcrypted \"Transcript\" Credential to employee");
   console.log(JSON.stringify(authcryptedTranscriptCredJson));

   return JSON.stringify(authcryptedTranscriptCredJson);
}

//This method will be invoked by the employee.
async function StoreCredentials(emplWalletConfig,emplWalletCred,emplClientKey,
    authcryptedTranscriptCredJson,transcriptCredRequestMetadataJson,clientTranscriptCredDef)
{
    indy.setProtocolVersion(2);
    ...
    console.log("\"Employee\" -> Authdecrypted \"Transcript\" Credential from client");
    //console.log(authcryptedTranscriptCredJson)
    let bufferOriginal = Buffer.from(JSON.parse(authcryptedTranscriptCredJson).data);
    console.log('bufferOriginal');
    console.log(bufferOriginal);

    //console.log('bufferOriginaljson');
    
    //let bufferOriginalNew = JSON.parse(bufferOriginal);
    //let bufferOriginalNew = Buffer.from(bufferOriginal);
    //console.log(bufferOriginalNew);
    //console.log(bufferOriginalNew);
    let authdecryptedTranscriptCredJson;
    
    await authDecrypt(emplWallet, emplClientKey, bufferOriginal).then(data=>{
        console.log(data);
        authdecryptedTranscriptCredJson = data;
    }).catch(e=>{
        console.log(e);
    });

    
    console.log("\"Employee\" -> Store \"Transcript\" Credential from client");
    await indy.proverStoreCredential(emplWallet, null, transcriptCredRequestMetadataJson,
        authdecryptedTranscriptCredJson, clientTranscriptCredDef, null).then(data=>{
